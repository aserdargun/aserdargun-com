...
export function learningDiagramLayout(applications) {
  ...
  for (const [code, layout] of Object.entries(PARENTS)) {
    ...
    const childNodes = children.map((child, index) => {
      ...
      const node = { app: child, cx, x: cx - width / 2, y, width, height: 50, role: code === "swi" ? "colony-lab" : child.kind === "tool" ? "learning-tool" : "practice-lab" };
...
export function renderLearningDiagram({ locale, data }) {
  ...
  const description = localized(locale,
    "Large boxes show the connected main applications. Smaller boxes are sub-applications, enclosed with their parent in a shared frame. AIA connects GPU and USL to LLM, then HNS, CTX, SEC and EVL. CTX feeds back to LLM. Assurance informs parallel LCL and CLD deployment; their outputs join below the deployment row into a single trunk line that continues to WFM and SWI, then ITL and ENG. DCL remains their shared decision laboratory, separate from the main flow. USL owns ADP, LLM owns TFL, HNS owns ARL, GPU owns POL for programming foundations and GEX for educational kernel execution, WFM owns WML, SWI owns ANT and BEE, ITL owns PDT, and ENG owns HEX. A dot marks the shared physical-AI junction.",
    "Büyük kutular birbirine bağlı üst uygulamaları gösterir. Küçük kutular alt uygulamalardır; üst uygulamalarıyla ortak çerçeve içindedir. AIA, GPU ve USL üzerinden LLM, HNS, CTX, SEC ve EVL’ye bağlanır. CTX, LLM’ye geri bildirim verir. Güvence katmanı paralel LCL ve CLD dağıtımına bilgi verir; LCL ve CLD çıktıları dağıtım satırının altında tek bir hatta birleşir ve WFM ile SWI’ye, ardından ITL ve ENG’ye ulaşır. DCL, ana akıştan ayrı olarak LCL ve CLD’nin ortak karar laboratuvarı olmaya devam eder. USL altında ADP, LLM altında TFL, HNS altında ARL, GPU altında programlama temelleri için POL ve eğitsel kernel yürütmesi için GEX, WFM altında WML, SWI altında ANT ve BEE, ITL altında PDT, ENG altında HEX bulunur. Nokta, ortak fiziksel AI bağlantısını gösterir.");
